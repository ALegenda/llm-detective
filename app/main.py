...
def case_details(cid:str,user=Depends(authenticate)):
    ...
    result['stage']='ready' if c['status']=='ready' else (cp.get('stage','outline') if cp.get('pipeline_version',1)>=2 else ('review' if 'blueprint' in cp else 'writing'))
    if result['stage']=='reader':result['stage']='reading'
    if result['stage'] in ['adjudication','fact_audit']:result['stage']='audit'
...
def command(aid:str,body:CommandInput,request:Request,user=Depends(authenticate)):
    ...
    with db.transaction() as con:
        ...
        if body.kind in ['object','travel']:
            try:(world.object_step if body.kind=='object' else world.travel_step)(json.loads(owned_case(a['case_id'],user)['blueprint']),json.loads(current['state']),payload)
