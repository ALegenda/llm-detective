...
class DiscoveryTurn(Model):
    ...
    clue_indices: list[int] = Field(min_length=1, max_length=4, description='Zero-based indices of ALL observations needed for this revised interpretation, including a comparison check if the claim relies on its result. Do not cite only its input sources while claiming an unperformed test.')
...
def contract_schema(b):
    ...
    source_check=create_model('SourceContract',__base__=ContractCheck,stage=(Literal['outline','world'],...))
    person_check=create_model('PersonContract',__base__=ContractCheck,stage=(Literal['outline','script'],...))
    author_check=create_model('AuthorContract',__base__=ContractCheck,stage=(Literal['outline'],...))
    return create_model('ExperienceContracts',__base__=Model,
        continuity=(author_check,...),discoveries=(author_check,...),grading=(author_check,...),
        **{c['id']:(source_check,...) for c in b['checks']},
        **{p['id']:(person_check,...) for p in b['people']})


CONTRACT_PROMPT = '''Verify each required contract, using concrete authored facts rather than a numeric quality rating.
For EACH f_N, check only its own observation_contract inputs. An inspect/read must describe this source alone; it cannot say an address/signature matches an unread other document. Quote the actual address/signature/mark instead. Comparing two sources requires their declared prior_observations; being supplied elsewhere in this audit is NOT a player prerequisite. Experiments use their listed instrument and cannot preselect the player's clock time. Do not require a prerequisite merely to read a document which itself quotes another source. Distinguish what a document CLAIMS from what the narrator asserts as independently verified. Every observation is canonical outline text; script cannot change it. On failure stage=outline for an incorrect source fact or unsupported physical effect, stage=world for missing declared prerequisite observations/instruments, never script for observation prose. Do not remove useful source details to hide a contradiction. certified_discoveries are real public results from engine replay: they establish where and inside which containers an object was found. An inspection need not repeat that already-recorded provenance. Do not require duplicate prose for a fact supplied by the engine.
For EACH n_N, verify that speakable account CLAIMS cover their OWN recent actions visible in the timeline or attributed records: what they did/saw and why they deliberately did it. A culprit may have a coherent authored lie or refusal, but innocent routine actions need an actual explanation, not blanket evasion or artificial amnesia. The actor never receives knowledge or private_context: these are author-only fields and cannot satisfy speakable coverage. Evaluate ungated claims as the initial conversational state, separately from evidence-gated claims. An ungated confession of the central concealed act fails script; case secrets in personality/interests fail outline because those fields are always supplied to the actor; a public alibi, denial or ordinary partial admission may be valid. At least one available ungated account per person must add a specific relevant personal observation or explanation rather than only repeating a document or saying they do not know. Routine conduct with a clear purpose from the public role and task, such as locking a display at closing, does not require a separately narrated motive. Require a specific personal reason for suspicious or consequential choices whose purpose would otherwise be missing; do not demand explanations of every mundane movement. No omniscient knowledge. If personal memory is absent repair outline; if known facts are absent from usable accounts repair script. Do not demand a confession or an answer about events the person never witnessed.
grading: compare all truth.criteria by their logical requirements. Each must earn credit for a distinct contribution; a detail that repeats the method or its experiment is duplicate weighting, even if phrased differently. Fail outline and remove/merge redundant details into the primary criterion; keep identity, method and motive distinct. Do not invent additional required details just to fill a quota.
...
discoveries: reject a source that explicitly packages the responsible person, deliberate method and hidden motive into one ready-made solution or written crime plan: that eliminates the planned inference even if other clues corroborate it. Require independent observations whose combination establishes the answer, not repeated confession documents. Verify both planned changes of interpretation against their explicitly resolved supporting_observations and certified_discoveries. Do not reinterpret numerical indices. They must have concrete obtainable evidence and be distinct. Neither may merely restate the initial assignment or require a new fact introduced only by the final truth. A refinement rather than a dramatic twist is valid. Respect the requested difficulty; hard needs competing plausible causal explanations distinguished by chronology and material comparison. Repair outline for absent or unsupported development. This checks specific authoring contracts, not subjective enjoyment; do not reject stylistic preferences.'''
...
SCRIPT_PROMPT = '''Write ONLY the player briefing, NPC testimony and hints for the FIXED mystery and compiled world. The material observations in compiled_world.checks are canonical and cannot be rewritten by this stage. Do not output observation fields or alter facts, causal truth, physical placements or evidence. Use the fixed observations as context for grounded conversations. Introduction: complete atmospheric 2-3 paragraphs stating concrete incident/discovery/time/assignment, no hidden culprit or undiscovered clues. objective identifies questions to solve; known_facts only public opening facts. Public participant context is concise THIRD-PERSON public briefing prose explaining relevance without guilt leakage; it is not a spoken NPC reply. Never put engine rules, availability of rooms, route topology, checks, IDs, evidence prerequisites or other implementation details into introduction/objective/known_facts. Describe real-world circumstances only.
...
def compile_world(o, raw_plan, script=None, language='ru'):
    if script is not None:script=script_schema(o).model_validate(script).model_dump()
    ...
    for i,c in enumerate(o['clues']):
        ...
        # Material observations have one author. Dialogue scripting cannot alter them.
...
def exercise_world(b, required_items=(), reverse=False):
    ...
    return {'actions':trace,'acquired':sorted(known),'recovered':sorted(set(required_items)&set(s['inventory'])),'minutes':s['minute'],'discoveries':[e for e in s['evidence'] if e.get('discovery')]}


def reader_schema(b, discoveries=()):
    return create_model('BlindReading',__base__=ReaderSolution,
        ...
        supporting_evidence=(list[Literal.__getitem__(tuple(c['id'] for c in b['checks'])+tuple(e['id'] for e in discoveries))],Field(min_length=2)))
...
def build(job, ai, settings):
    ...
    discoveries=cp['certificate']['forward']['discoveries']
    discovery_contracts=[{k:v for k,v in d.items() if k!='clue_indices'} |
        {'supporting_observations':[contracts[i] for i in d['clue_indices']]} for d in outline['discoveries']]
    readiness=stage_call('contracts','story_contract_audit',CONTRACT_PROMPT,
        {'truth':b['truth'],'observations':contracts,'people':b['people'],
         'discoveries':discovery_contracts,'certified_discoveries':discoveries,'opening':b['introduction'],
